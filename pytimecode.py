"""Module for manipulating SMPTE timecode. Supports 60, 59.94, 50, 30, 29.97,
25, 24, 23.98 frame rates in drop and non-drop where applicable,
and milliseconds. It also supports
operator overloading for addition, subtraction, multiplication, and division.

iter_return sets the format that iterations return, the options are "tc" for
a timecode string,
"frames" for a int total frames, and "tc_tuple" for a tuple of ints in the
following format,
(hours, minutes, seconds, frames).

Notes: *There is a 24 hour SMPTE Timecode limit, so if your time exceeds that
limit, it will roll over.
       *2 PyTimeCode objects of the same frame rate is the only supported way
       to combine PyTimeCode objects,
...
class PyTimeCode(object):
    def __init__(self, framerate, start_timecode=None, frames=None,
                 drop_frame=False, iter_return="tc"):
        """frame rate can be string '60', '59.94', '50', '30', '29.97', '25',
        '24', '23.98', or 'ms'"""
    ...
    def tc_to_frames(self):
        ...
        frames = (((self.hrs * 3600) + (
        self.mins * 60) + self.secs) * self.int_framerate) + self.frs
        if self.drop_frame:
    ...
    def frames_to_tc(self, frame_only=False):
        ...
        self.mins = (frames % (3600 * self.int_framerate)) / (
        60 * self.int_framerate)
        self.secs = ((frames % (3600 * self.int_framerate)) % (
        60 * self.int_framerate)) / self.int_framerate
        self.frs = ((frames % (3600 * self.int_framerate)) % (
        60 * self.int_framerate)) % self.int_framerate
        if self.drop_frame:
            if self.frs == 0 and (
                self.mins % 10): #tests to see if frames is 0 and if minutes
                # is not an even 10, 20, 30, etc
    ...
    def calc_drop_frames(self, frame_only=False):
        if frame_only:
            ...
            mins = (self.frames % (3600 * self.int_framerate)) / (
            60 * self.int_framerate)
            if mins % 10: #if the minutes is not a multiple of 10,
            # there needs to be one more drop frame unit, 2 or 4
                extra = 1
            else:
                extra = 0
            if self.framerate == '59.94':
                return (hours * 6 * 36) + ((mins / 10) * 36) + (
                mins % 10 * 4) + (extra * 4)
            elif self.framerate == '29.97':
                return (hours * 6 * 18) + ((mins / 10) * 18) + (
                mins % 10 * 2) + (extra * 2)
        elif self.framerate == '59.94':
            return (self.hrs * 6 * 36) + ((self.mins / 10) * 36) + (
            self.mins % 10 * 4)
        elif self.framerate == '29.97':
            return (self.hrs * 6 * 18) + ((self.mins / 10) * 18) + (
            self.mins % 10 * 2)
        else:
            raise PyTimeCodeError('Drop frame with ' + self.framerate + \
                                  'fps not supported, only 29.97 & 59.94.')
    ...
    def __add__(self, other):
        ...
        if type(other) == PyTimeCode:
            ...
            raise PyTimeCodeError(
                'Type ' + str(type(other)) + ' not supported for arithmetic.')
        newtc = PyTimeCode(self.framerate, start_timecode=None,
                           frames=added_frames, drop_frame=self.drop_frame)
        return newtc
    ...
    def __sub__(self, other):
        ...
        if type(other) == PyTimeCode:
            ...
            raise PyTimeCodeError(
                'Type ' + str(type(other)) + ' not supported for arithmetic.')
        newtc = PyTimeCode(self.framerate, start_timecode=None,
                           frames=subtracted_frames, drop_frame=self.drop_frame)
        return newtc
    ...
    def __mul__(self, other):
        ...
        if type(other) == PyTimeCode:
            ...
            raise PyTimeCodeError(
                'Type ' + str(type(other)) + ' not supported for arithmetic.')
        newtc = PyTimeCode(self.framerate, start_timecode=None,
                           frames=mult_frames, drop_frame=self.drop_frame)
        return newtc
    ...
    def __div__(self, other):
        ...
        if type(other) == PyTimeCode:
            ...
            raise PyTimeCodeError(
                'Type ' + str(type(other)) + ' not supported for arithmetic.')
        newtc = PyTimeCode(self.framerate, start_timecode=None,
                           frames=div_frames, drop_frame=self.drop_frame)
        return newtc
